...
if (btn_extract) {
  btn_extract.onclick = async function () {
    ...
    document.querySelector("textarea[name='create-img']").innerHTML = response.text;
...
if (form_openai) {
  form_openai.onsubmit = async function (e) {
    ...
    let create = extraction_type ? formData.get("create") : formData.get("create");
    ...
    // Pass Data to OpenAI
    const response = await window.axios.openAI(create, tools_type);
    ...
    const db_response = await window.axios.store('post', 'prompts', {
        text: create,
        result: result,
